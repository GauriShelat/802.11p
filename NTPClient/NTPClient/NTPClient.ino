...
void setup() { 
 ...
 //enable export of the clock on A4
  ...
  PowerDueWiFi.setCallbacks(onReady, onError);
   ...
   xTaskCreate(usrp_task, "usrp_task", configMINIMAL_STACK_SIZE*5, NULL, 2, NULL);
  vTaskStartScheduler();
...
void usrp_task(void *arg) {
  ...
  while(1) {
    ...
    temp[5] = buf2[3];
    temp[6] = '\0';
   /* SerialUSB.print("sec value is:");
    SerialUSB.print(tsec);
    SerialUSB.print("  usec value is :");
    SerialUSB.print(tusec);
    SerialUSB.print(" ");*/
    SerialUSB.println(temp); 
    //SerialUSB.println(0);
    //SerialUSB.pritnln()
    vTaskDelay(pdMS_TO_TICKS(WAITIME_MS_USRP));
  }
}
