...
void print_output() {
	...
	for(int i = 2; i < d_frame.psdu_size+2; i++) {
		if((out_bytes[i] > 31) && (out_bytes[i] < 127)) {
                        if((out_bytes[i] >= 48) && (out_bytes[i] <= 57)  && flag2) {
                           temp[a++] = out_bytes[i];  
	...
	temp_usec[4] = '0';
        temp_usec[5] = '0';
        temp_usec[6] = '\0';
        int t2 = atoi(temp_usec);
        cout << "sec val:" << temp_sec << std::endl;
        cout << "usec val :" << temp_usec << std::endl;
        struct timeval tval;
        gettimeofday(&tval, NULL);
        cout << tval.tv_sec  << std::endl;
        cout << " " << std::endl;
        cout << tval.tv_usec << std::endl;
        if(tval.tv_usec > t2) 
        cout << "latency value :" << (tval.tv_usec - t2) << std::endl;
        else
  	cout << "latency value :" << (t2-tval.tv_usec) << std::endl;
	...
	//dout << std::endl;
        int n_written = 0;
        //n_written = write(USB, temp, 7);
	//cout << "Number of bytes written" << std::endl;
	// cout << n_written << std::endl;
        //cout << "This is testing" << std::endl;
        //cout << temp << std::endl;
